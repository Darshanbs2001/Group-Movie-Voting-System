...
class UserServiceImpTest {
    ...
    @BeforeEach
    void setup(){
        userToSave = new UserDto("vinay b s", "Vi@9141525275", "vinay@gmail.com");
        user=new User();
        user.setUserName("vinay b s");
    ...
    public void testcreateUser() {
        ...
        when(encoder.encode(Mockito.anyString())).thenReturn("Encoded string");
    ...
    public void test_loginUser_Success() {
        ...
        response.setUser(userToSave);

        // Assert
        assertNotNull(response);
        assertEquals("mocked-jwt-token", response.getJwt());
        assertEquals("vinay@gmail.com", response.getUser().getEmail());
    }
